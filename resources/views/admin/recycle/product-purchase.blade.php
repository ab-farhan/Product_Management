    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header">
                <div class="row">
                    <div class="col-md-8">
                        <h3 class="card-title card_top_title"><i class="fa fa-gg-circle"></i> Recycle All Purchase</h3>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="table-responsive">
                            <table id="alltableinfo" class="table table-bordered custom_table mb-0">
                                ...
                                <tbody>
                                    @php
                                        $all=App\ProductPurchase::where('purchase_status',0)->orderBy('purchase_id','DESC')->get();
                                    @endphp
                                    @foreach($all as $data)
                                    <tr>
                                        ...
                                        <td>{{$data->purchase_qunatity}}</td>
                                        <td>{{$data->purchase_unit_price}}</td>
                                        <td>{{$data->purchase_total_price}}</td>
                                        <td>
                                            <a href="javaScript:void(0)" title="restore" id="restore" data-toggle="modal" data-target="#restoreModal" data-id="{{$data->purchase_id}}"><i class="fa fa-refresh fa-lg restore_icon"></i></a>
...
<div id="restoreModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/product/purchase/restore')}}">
              @csrf
              <div class="card mb-0">
                <div class="card-header">
...
<div id="softDelModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none">
    <div class="modal-dialog">
        <div class="modal-content p-0 b-0">
            <form method="post" action="{{url('dashboard/product/purchase/delete')}}">
              @csrf
              <div class="card mb-0">
                <div class="card-header">
                    <h3 class="card-title float-left"><i class="fa fa-gg-circle"></i> Confirm Message</h3>
